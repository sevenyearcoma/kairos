...
import { $tasks, $events, $language, $knowledgeBase, addTask, addEvent, deleteTask, editTask, getLocalDateStr } from '../stores/app';
...
const TasksView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleEnd = (e: TouchEvent) => {
      ...
      if (dropZone) {
        ...
        if (newP && newP !== touchDragItem.priority) editTask(touchDragItem.id, { priority: newP });
      }
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addTask(newTaskTitle, res.category || newTaskCategory, '', undefined, 'none', res.priority || 'normal');
      setNewTaskTitle('');
      setShowInput(false);
    } catch {
      addTask(newTaskTitle, newTaskCategory, '', undefined, 'none', 'normal');
      setNewTaskTitle('');
      setShowInput(false);
  ...
  const renderStack = (priority: TaskPriority) => {
    ...
    return (
      <div key={priority} data-drop-zone={priority} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { const tid = e.dataTransfer.getData('text'); editTask(tid, { priority }); }} className={`flex flex-col rounded-[2rem] shadow-sm transition-all ${style.bg} border ${style.border}`}>
